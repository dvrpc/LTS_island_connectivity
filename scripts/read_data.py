...
def import_data():
    """
    Import the most recent data from all sources
    """
    print("creating necessary extensions on database, importing data...")

    base_url = "https://arcgis.dvrpc.org/portal/rest/services/"

    end_url = "FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson"

    data_to_import = [
        ("municipalboundaries", "Boundaries", "MunicipalBoundaries", True),
        # ("circuittrails", ""),
        # ("lts_network", ""),
        # ("pedestriannetwork_lines", "https://arcgis.dvrpc.org/portal/rest/services/Transportation/pedestriannetwork_lines/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson"),
        # ("pedestriannetwork_gaps", ""),
        # ("crash_newjersey", ""),
        # ("crash_nj_pedestrians", ""),
    ...
    for sql_tablename, schema, layer, explode in data_to_import:
        source_url = base_url + schema + "/" + layer + "/" + end_url
        print(f"Extracting the {layer} layer from the {schema} schema.")
        kwargs = {
            "filepath": source_url,
